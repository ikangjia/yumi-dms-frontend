<template>
  <div class="datasource-main">
    ...
    <DataSourceSaveDrawer ref="datasourceSaveDrawer" @refreshDataSourceTable="doSearch" />
      ...
      <el-table-column label="操作" min-width="400px">
        <template #default="scope">
          <el-button
            size="small"
            type="warning"
            @click="openUpdateDataSourceDrawer(scope.$index, scope.row)"
            >编辑
          </el-button>
          <el-button size="small" type="primary" @click="testConnection(scope.$index, scope.row)"
            >连通性测试</el-button
          >
          <el-button size="small" type="info" @click="goToGoodPlace(scope.$index, scope.row)"
            >进入控制台</el-button
          >
          <el-button size="small" type="danger" @click="deleteDataSource(scope.$index, scope.row)"
            >删除</el-button
          >
        </template>
      </el-table-column>
...
function doSearch() {
  ...
  datasourceApi.doSearch(params).then((res) => {
    if (res.code === 0) {
...
function testConnection(index, row) {
  datasourceApi.testConnection(row).then((res) => {
    if (res.code === 0) {
...
function deleteDataSource(index, row) {
  ElMessageBox.confirm('确定要删除数据源 [' + row.name + '] 吗？')
    .then(() => {
      datasourceApi.deleteDataSource(row.id).then((res) => {
