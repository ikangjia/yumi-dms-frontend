    ...
    <div class="mysql-aside">
      ...
      <div
        :style="{
          ...
          width: '130px',
          background: 'white',
          'box-shadow': '2px 2px 14px rgba(0, 0, 0, .06), -2px -2px 4px rgba(0, 0, 0, .06)',
          'text-align': 'center'
        }"
        v-show="menuVisible"
      ...
      >
        <template v-for="m in menuData">
          <el-button class="menu-button" :id="m.id" @click="handleRightClick(m.id, m.type)">
            {{ m.title }}
          </el-button>
...
function rightClick(event, data, node, target) {
  menuVisible.value = false
  currentDatabaseName.value = data.labelName
      ...
      switch (value) {
        ...
        case 'dropDatabase':
          ElMessageBox.confirm('确认删除么', '提示', {
            ...
            .then(() => {
              mysqlApi
                .dropDatabase(currentDatabaseName, { datasourceId: datasourceId.value })
                .then((res) => {
                  ElMessage({
                    type: 'success',
                    message: '删除成功'
