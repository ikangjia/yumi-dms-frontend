<template>
  <el-drawer v-model="isShow" title="创建数据库" size="30%" direction="rtl" @open="init">
    <el-form :model="databaseCreate" label-width="80px">
      ...
      <el-form-item label="字符集:">
        <el-select v-model="databaseCreate.characterSet" placeholder="默认" @change="initCollation">
          <el-option
            v-for="(item, index) in characterSetList"
            :label="item.characterSet"
            :value="item.characterSet"
          />
        </el-select>
      </el-form-item>
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="saveDatabase"> 确认 </el-button>
      </span>
    </template>
  </el-drawer>
</template>
...
function getCharacterSets() {
  mysqlApi.getCharacterSets({ datasourceId: datasourceId }).then((res) => {
    if (res.code === 0) {
...
function initCollation() {
  mysqlApi
    .getCollations({ datasourceId: datasourceId, characterSet: databaseCreate.value.characterSet })
    .then((res) => {
      if (res.code === 0) {
...
function saveDatabase() {
  mysqlApi.createDatabase(databaseCreate.value).then((res) => {
    if (res.code === 0) {
